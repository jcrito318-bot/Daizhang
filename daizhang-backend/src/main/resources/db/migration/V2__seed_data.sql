-- ============================================
-- V2__seed_data.sql - 种子数据 (用户/角色/菜单/科目/凭证字)
-- ============================================
-- 来源: data-h2.sql
-- 变更:
--   1. MERGE INTO 转换为 INSERT (Flyway 迁移仅执行一次, 无需幂等;
--      同时 INSERT 兼容 H2 和 MySQL)
--   2. 排除 ai_accounting_rule 数据 (移至 V4)
--   3. 排除 tax_benchmark 数据 (移至 V7)
-- ============================================

-- ============================================
-- 代账系统初始化数据脚本 (H2/MySQL 兼容)
-- 使用 INSERT 语法 (Flyway 迁移仅执行一次)
-- ============================================
